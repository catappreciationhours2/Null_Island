<script>
  ...
  function drawRadar() {
    ...
    vals.forEach((v, i) => {
      const a = (i / N) * Math.PI * 2 - Math.PI / 2;
      const r = maxR * Math.min(v, 1.5);
      ctx.beginPath(); ctx.arc(cx + Math.cos(a) * r, cy + Math.sin(a) * r, v > 1 ? 4 : 3, 0, Math.PI * 2);
    ...
    ATTRS.forEach((label, i) => {
      const a = (i / N) * Math.PI * 2 - Math.PI / 2;
      const x = cx + Math.cos(a) * (maxR + 18), y = cy + Math.sin(a) * (maxR + 18);
  ...
  // getBoundingClientRect accounts for CSS zoom already via the scale ratio
  // @ts-ignore
  function toCanvas(e) {
    // @ts-ignore
  ...
  // ── Arrow key movement (8 directions via Shift) ──
  // @ts-ignore
  function onCanvasKeyDown(e) {
    ...
    if (moves[e.key]) {
      ...
      // Shift → diagonal: combine perpendicular axes
      if (e.shiftKey) {
  ...
  onMount(async () => {
    ...
    fuseInstance = new Fuse(appState.taskHistory, { keys: ['title','difficulty','tags'], threshold: 0.35 });
  });

  $effect(() => {
    if (!historyQuery.trim()) { searchResults = appState.taskHistory.slice(0, 50); return; }
    if (fuseInstance) {
      // @ts-ignore
      const FuseClass = fuseInstance.constructor;
      const fresh = new FuseClass(appState.taskHistory, { keys: ['title','difficulty','tags'], threshold: 0.35 });
      // @ts-ignore
      searchResults = fresh.search(historyQuery).map(r => r.item).slice(0, 50);
    }
  });

  $effect(() => { appState.taskHistory.length; if (!historyQuery.trim()) searchResults = appState.taskHistory.slice(0, 50); });
  ...
  {#if profileTab === 'overview'}
    <div class="profile-grid">
        ...
        <div class="char-area">
          ...
          {#if selectedEq}
            <div class="item-controls">
              ...
              <!-- Only rotate + scale buttons remain; movement is mouse/keyboard only -->
              <div class="ctrl-row">
      ...
      <div class="card radar-card">
        ...
        <div class="radar-sub">Surpass 100 to overflow the pentagon</div>
      </div>
        ...
        {#each ATTR_KEYS as key, i}
          ...
          <div class="attr-row">
            ...
            <div class="attr-track">
              ...
              {#if val > 100}<div class="attr-overflow" style="width:{Math.min(val-100,50)}%"></div>{/if}
            </div>
      ...
      <div class="card monthly-card">
        ...
        <div class="month-stats">
          ...
          <div class="month-row"><span>Top attr</span><span>Focus</span></div>
...
.char-canvas {
  ...
  border-radius:var(--radius); flex-shrink:0; width:160px; height:160px;
}
...
.ctrl-btn {
  padding:3px 9px; font-size:12px; background:var(--bg3);
  border:1px solid var(--border); border-radius:var(--radius);
  color:var(--text2); cursor:pointer; font-family:var(--font-mono); transition:all .12s;
}
...
.attr-val { font-size:9px; font-family:var(--font-mono); color:var(--accent); width:22px; text-align:right; }
...
.inv-slot {
  ...
  cursor:pointer; transition:all .15s ease; min-height:64px; justify-content:center; position:relative;
}
...
/* Motivation button */
.motivate-btn {
  padding:10px; background:var(--bg2); border:1px dashed var(--border); border-radius:var(--radius-lg);
  color:var(--text3); font-family:var(--font-ui); font-size:12px; cursor:pointer; transition:all .2s; text-align:center; margin-top:4px;
}
...
.search-input {
  ...
  color:var(--text); font-family:var(--font-ui); font-size:13px; padding:7px 30px 7px 10px; outline:none; transition:border-color .15s;
}
